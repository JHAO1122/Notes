...
??? proof "Proof of Theorem 3.14: Blocking Technique (Click to expand)"
    ...
    **Step 1: Construct Large and Small Blocks**
    ...
    Let the large block length be $k = [n^{1/3}]$, and the small block length be $m$.
    ...
    **Step 5: Central Limit Theorem for Large Blocks**
    ...
    Thus, the Lindeberg condition holds. By the Lindeberg-Feller CLT, we have $S_n' / \sigma_n' \xrightarrow{d} N(0,1)$.
    Combining with Slutsky's theorem, the final conclusion is proven. $\square$
...
For independent and identically distributed sequences with finite variance, their standardized sums converge to the normal distribution. A natural question is: **If we relax the condition of finite variance, to which non-degenerate distributions can the standardized sums still converge?**
...
The family of non-degenerate stable distributions is exactly equivalent to the family of all possible **non-degenerate limit distributions** of sums of i.i.d. random variables after appropriate centering and scaling.
...
!!! success "Theorem 3.22 and Corollary 3.23 (Characterization of Domains of Attraction)"
    ...
    1. **Domain of attraction of the normal distribution $DA(2)$:** $F \in DA(2)$ if and only if $L(x) = \int_{|y|<x} y^2 dF(y)$ is a slowly varying function.
       This is equivalent to the tail probability satisfying:
    ...
    2. **Domain of attraction of the $\alpha < 2$ stable distribution $DA(\alpha)$:**
       $F \in DA(\alpha)$ if and only if its left and right tails exhibit Pareto-type decay:
...
**Proposition 3.25 (Normalization constant $b_n$):**
For $F \in DA(\alpha)$, the normalization constant $b_n$ can be chosen as the unique solution to the following equation:
