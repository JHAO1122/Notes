# 🛠️ Quick Reference

This is my **concept quick reference**. In the process of studying mathematical statistics and conducting research, I have found that personal memory can be quite unreliable, often prone to misremembering, omitting, or forgetting details. This section aims to compile the frequently used yet easily forgotten core definitions, theorems, and their underlying logic from courses such as probability & statistics and linear algebra, facilitating quick retrieval for my own reference.

---

## 🧭 Knowledge Map Index

This quick reference manual is divided into the following core areas based on knowledge modules:

### [🎲 Probability Theory](prob.md)

### [📖 Mathematical Statistics](stats.md)

### [📏 Foundations of Measure Theory](measure.md)

### [🔢 Linear Algebra](linalg.md)

### [🌊 Stochastic Processes](stochastic.md)

---

## 💡 Disclaimer

!!! quote "About the Nature of this Manual"
    1. **Dynamic Evolution**: This manual is not a replica of textbooks, but rather a summary of ambiguous knowledge points I encounter during my daily studies, research, and interview preparation. It will continuously iterate as my understanding deepens.
    2. **Corrections Welcome**: Since the content is primarily organized by myself, typos or misunderstandings are inevitable. If you find any issues while reading, please feel free to remind me to correct them via [GitHub Issue](https://github.com/jhao1122/Notes/issues).