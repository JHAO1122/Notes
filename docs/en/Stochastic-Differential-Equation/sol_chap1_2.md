...
# 📝 Detailed Solutions to Exercises: Conditional Expectation and Properties of Brownian Motion

!!! abstract "About This Page"
    This page contains detailed solutions to key exercises from Chapter 1 (Independence, Conditional Expectation) and Chapter 2 (Brownian Motion and Its Properties) of the *Stochastic Differential Equations* course. All solutions are presented in collapsible boxes—click to view the detailed derivations.
...
**Problem:**
Let the probability density function of the random variable $X$ be $f(x) = ax(1-x), x \in (0,1)$. For all other values, $f$ is zero.
...
??? success "Solution (click to expand)"
    ...
    **(1) Finding the constant $a$**
    ...
    By the normalization property of the probability density function, the integral over the entire space equals 1:
    ...
    **(2) Finding the probability density function of $Y=X^3$**
    ...
    Substitute the probability density function of $X$ and compute the integral:
...
??? success "Solution (click to expand)"
    ...
    **(1) Proof:**
    ...
    [cite_start]We use the standard machine approach from measure theory for a rigorous proof [cite: 1520]:
    ...
    **Step 1 (Indicator functions):** Let \(f(x,y) = I_A(x)I_B(y)\), where \(A, B\) are Borel sets.
    ...
    The two are equal, so the result holds for indicator functions.
    ...
    **Step 3 (Non-negative measurable functions):** For any non-negative measurable function, there exists a monotone increasing sequence of simple functions that converges to it. By the Monotone Convergence Theorem, the conclusion holds for non-negative measurable functions [cite: 1520].
    
    **Step 4 (Bounded continuous functions):** Any bounded continuous function can be decomposed into positive and negative parts \(f = f^+ - f^-\), and by definition it is Lebesgue integrable. Therefore, the original formula holds for all bounded continuous functions. QED [cite: 1520].
    ...
    **(2) Proof:**
    ...
    Transform the integration domain equivalently as \(u \leqslant x - Y\):
...
??? success "Solution (click to expand)"
    ...
    Since \( \chi_B \) can only take values 0 or 1, the σ-algebra generated by it is very simple: \( \sigma(\chi_B) = \{\emptyset, \Omega, B, B^c\} \).  
    Therefore, the conditional expectation \( \mathbb{E}[\chi_A|\chi_B] \) must be \( \sigma(\chi_B) \)-measurable, meaning it must be constant on \( B \) and \( B^c \). Let us denote:
...
**Problem:**  
Let $\mathcal{V}_1$ and $\mathcal{V}_2$ be two independent $\sigma$-fields, and let $X$ be an integrable random variable. Prove that:  
...
??? success "Solution (click to expand)"
    ...
    **Proof:**  
    ...
    Since it is given that $\mathcal{V}_1$ and $\mathcal{V}_2$ are independent $\sigma$-fields, the $\mathcal{V}_1$-measurable random variable $\mathbb{E}(X|\mathcal{V}_1)$ is naturally independent of the $\sigma$-field $\mathcal{V}_2$.
    
    By the property of conditional expectation with respect to an independent $\sigma$-field (if a random variable $Z$ is independent of a $\sigma$-field $\mathcal{G}$, then $\mathbb{E}[Z|\mathcal{G}] = \mathbb{E}[Z]$), we can remove the conditioning on $\mathcal{V}_2$ [cite: 1529]:
    ...
    Combining the two equations, the original statement is proved [cite: 1529]. $\square$
...
**Problem:**  
Let $X$ and $\{X_n\}$ be a sequence of random variables on $(\Omega, \mathcal{F}, \mathbb{P})$, and for a fixed $1 \leqslant p < \infty$, assume $\mathbb{E}[|X_n|^p] < +\infty$. Suppose $\lim_{n\to\infty}\mathbb{E}[|X_n - X|^p] = 0$. Prove that for any sub-$\sigma$-field $\mathcal{V} \subset \mathcal{F}$, we have  
...
??? success "Solution (click to expand)"
    ...
    **Proof:**  
    ...
    Taking unconditional expectation on both sides of the above inequality and applying the law of total expectation [cite: 1530, 1531]:
    ...
    Taking the limit as $n \to \infty$ on both sides of the inequality. Since it is given that $\lim_{n\to\infty}\mathbb{E}[|X_n - X|^p] = 0$, and the left-hand side expectation is always nonnegative, by the squeeze theorem [cite: 1532]:
...
**Problem:**
Take $\Omega = \{1, 2, \cdots, 7, 8\}$, $\mathcal{F} = 2^\Omega$ (i.e., the $\sigma$-algebra consisting of all subsets of $\Omega$). For $i \leqslant 4$, $\mathbb{P}(\{i\}) = 1/10$, and for $i > 4$, $\mathbb{P}(\{i\}) = 3/20$.
Define $X = \chi_{\{1,2,3,4\}} + 2\chi_{\{5,6,7,8\}}$, $Y = \chi_{\{1,5\}} + 2\chi_{\{2,3,4,6,7,8\}}$.
$\mathcal{V}$ is the $\sigma$-algebra generated by $\{1,2\}$ and $\{3,4\}$, and $\mathcal{H}$ is the $\sigma$-algebra generated by $\{1,2,3,4\}$.
Compute: (1) $X\mathbb{E}[Y]$; (2) $\mathbb{E}[\mathbb{E}[XY|\mathcal{V}]|\mathcal{H}]$.

??? success "Solution (click to expand)"

    **Preparation: Clarify the probability structure**
    
    * $\mathbb{P}(\{1,2,3,4\}) = 4 \times \frac{1}{10} = \frac{2}{5}$
    * $\mathbb{P}(\{5,6,7,8\}) = 4 \times \frac{3}{20} = \frac{3}{5}$
    * It is easy to verify that the two core atoms of $\mathcal{H} = \sigma(\{1,2,3,4\})$ are precisely $\{1,2,3,4\}$ and $\{5,6,7,8\}$. The partition of $\mathcal{V} = \sigma(\{1,2\}, \{3,4\})$ is finer, and clearly $\mathcal{H} \subset \mathcal{V}$.

    <br>

    **(1) Compute $X\mathbb{E}[Y]$**
    ...
    **(2) Compute $\mathbb{E}[\mathbb{E}[XY|\mathcal{V}]|\mathcal{H}]$**
    
    Here we use the extremely elegant smoothing property (Tower Property) to directly reduce dimensions, avoiding cumbersome double conditional expectation calculations [cite: 1533].
    Since $\mathcal{H} \subset \mathcal{V}$, the smaller $\sigma$-algebra plays a decisive role on the outside:
    ...
    * On $\{5,6,7,8\}$: $\mathbb{E}[Y|\{5,6,7,8\}] = \frac{1\cdot P(\{5\}) + 2\cdot P(\{6,7,8\})}{P(\{5,6,7,8\})} = \frac{3/20 + 18/20}{12/20} = \frac{7}{4}$
    ...
    Surprisingly, we find that $\mathbb{E}[Y|\mathcal{H}] = 7/4$ holds constant on both partitions. Hence:
...
??? success "Solution (click to expand)"
    ...
    **Proof:**
    [cite_start]We need to rigorously verify the three core properties of a martingale [cite: 1546, 1547]:
    
    **1. Integrability:**
    Using the contraction property of conditional expectation (the absolute value case of Jensen's inequality):
    ...
    **2. Adaptability:**
    By the measure-theoretic definition of conditional expectation $\mathbb{E}[X|\mathcal{F}(t)]$, it is naturally $\mathcal{F}(t)$-measurable. Therefore, the process $\{X(t)\}$ is adapted to the filtration $\{\mathcal{F}(t)\}_{t \ge 0}$.
    ...
    **3. Martingale Property:**
    ...
    Combining these three points, the process $X(t)$ constructed as the projection of a single integrable variable onto different information levels must be a martingale (known as a Doob martingale in stochastic analysis). $\square$
...
**Problem:**
Let $W(t)$ be a one-dimensional Brownian motion. Prove that for any fixed $s>0$, $W(t+s)-W(s)$ is a Brownian motion; and for any positive number $c$, $cW(t/c^2)$ is also a Brownian motion.
...
**Problem:**  
Let \( W(t) \) be a one-dimensional Brownian motion, and define  
\[
\tilde{W}(t) = \begin{cases} 
tW(1/t), & t > 0, \\
0, & t = 0.
\end{cases}
\]  
...
??? success "Solution (click to expand)"
    ...
    For \( 0 < s < t \), we rewrite the increment by adding and subtracting terms to isolate independent components [cite: 1557, 1558]:

    \[
    \tilde{W}(t) - \tilde{W}(s) = tW\left(\frac{1}{t}\right) - sW\left(\frac{1}{s}\right)
    \]

    By adding and subtracting \( sW(1/t) \), we separate:

    \[
    = (t-s)W\left(\frac{1}{t}\right) - s\left(W\left(\frac{1}{s}\right) - W\left(\frac{1}{t}\right)\right)
    \]

    Note that \( 1/t < 1/s \). Using the independent increments property of Brownian motion, the random variable \( W(1/t) = W(1/t) - W(0) \) is independent of the increment \( W(1/s) - W(1/t) \).

    Since they are independent, the variances add directly [cite: 1561]:

    \[
    \operatorname{Var}(\tilde{W}(t) - \tilde{W}(s)) = (t-s)^2 \operatorname{Var}\left( W\left(\frac{1}{t}\right) \right) + s^2 \operatorname{Var}\left( W\left(\frac{1}{s}\right) - W\left(\frac{1}{t}\right) \right)
    \]

    Substituting the variance formula \( \operatorname{Var}(W(u)) = u \):
    ...
    By linearity of expectation, \( \mathbb{E}[\tilde{W}(t) - \tilde{W}(s)] = 0 - 0 = 0 \) [cite: 1562].  
    Since it is a linear combination of jointly normal random variables, it must be normally distributed.

    Thus, we conclude: \( \tilde{W}(t) - \tilde{W}(s) \sim N(0, t-s) \).
...
??? success "Solution (click to expand)"
    ...
    Expand both sides as Taylor series around $\lambda = 0$:

    Left-hand side by linearity of expectation:

    $$
    \mathbb{E}[e^{\lambda W(t)}] = \sum_{m=0}^{\infty} \mathbb{E}[W^m(t)] \frac{\lambda^m}{m!}
    $$

    Right-hand side by expanding the exponential function:
    ...
    Compare coefficients of $\lambda^{2k}$ on both sides:

    $$
    \frac{\mathbb{E}[W^{2k}(t)]}{(2k)!} = \frac{t^k}{2^k k!}
    $$

    Rearranging yields the desired result [cite: 1571]:

    $$
    \mathbb{E}[W^{2k}(t)] = \frac{(2k)! t^k}{2^k k!}
    $$

    *(Note: For odd-order moments, comparing coefficients shows that the right-hand side has no odd-power terms, so $\mathbb{E}[W^{2k+1}(t)] = 0$.)*
...
??? success "Solution (click to expand)"
    ...
    Therefore, $\forall t, s \in \mathbb{R}$, $\mathbb{E}[U(t)U(s)] = e^{-|t-s|}$, which completes the proof.
...
??? success "Solution (click to expand)"
    ...
    **Step 2: Strengthening the moment bound (for Borel–Cantelli)**  
    If we only use the second-moment Chebyshev inequality, we obtain $P(|Y_m| \ge \epsilon) \le \frac{1}{m\epsilon^2}$ [cite: 1583]. However, $\sum_{m=1}^{\infty} \frac{1}{m}$ is the harmonic series, which does not converge, so we cannot directly apply the B–C lemma.  
...
??? success "Solution (click to expand)"
    ...
    **Proof (1): \(W(t)^2 - t\) is a martingale**
    ...
    Let \(0 \leqslant s < t\). We need to show \(\mathbb{E}[W(t)^2 - t | \mathcal{F}(s)] = W(s)^2 - s\).
    Using the identity \(W(t) = (W(t) - W(s)) + W(s)\) to expand the square:
    ...
    * Third term: \(W(s)^2\) is \(\mathcal{F}(s)\)-measurable and equals itself directly.
    ...
    **Proof (2): \(\exp\left(\lambda W(t) - \frac{1}{2}\lambda^2 t\right)\) is a martingale (geometric Brownian martingale)**
    ...
    Again let \(0 \leqslant s < t\). Using the exponential splitting:
    ...
    Substituting this expectation into the expression to be proved [cite: 1587]:
...
??? success "Solution (click to expand)"
    ...
    [cite_start]This is a highly representative stochastic process integral calculation, with the core idea being to use Fubini's theorem to interchange the order of expectation and integration [cite: 1588].

    First, express the square term as a double Riemann integral:
    ...
    Since the covariance function of Brownian motion is $\mathbb{E}[W(u)W(v)] = \min(u, v)$, substitute this into the above:

    $$
    = \int_0^t \int_0^t \min(u, v) du dv
    $$

    Because $\min(u,v)$ is not differentiable along the diagonal $u=v$, we split the integration region $[0,t] \times [0,t]$ along the diagonal into two parts: the region $u \leqslant v$ and the region $u > v$ [cite: 1589, 1590]:

    $$
    = \int_0^t dv \int_0^v u du + \int_0^t du \int_0^u v dv
    $$

    Since these two regions are completely symmetric, compute one and multiply by 2:

    $$
    = 2 \int_0^t \left( \int_0^v u du \right) dv = 2 \int_0^t \frac{1}{2}v^2 dv = \int_0^t v^2 dv = \frac{1}{3}v^3 \Bigg|_0^t = \frac{t^3}{3}
    $$

    Thus, the result is proven: $\mathbb{E}[X^2(t)] = \frac{t^3}{3}$.