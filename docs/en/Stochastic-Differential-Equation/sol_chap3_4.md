...
??? success "Solution (click to expand)"
    ...
    **Step 3: Determine the distribution type and density function**
    ...
    Therefore, the random variable follows a normal distribution with mean 0 and variance $\sigma^2 = \int_0^T g^2(t) dt$:
...
??? success "Solution (click to expand)"
    ...
    *Note: Compared to ordinary calculus $\int x^2 dx = \frac{1}{3}x^3$, the Itô integral includes an additional correction term $-\int_0^T W(t) dt$ arising from the quadratic variation term.*
...
??? success "Solution (click to expand)"
    ...
    This problem utilizes the Itô isometry and basic properties of Lebesgue integration from real analysis.
