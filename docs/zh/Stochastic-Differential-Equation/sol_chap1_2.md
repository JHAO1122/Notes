...
??? success "解答 (点击展开)"
    ...
    **(1) 证明 $B(t) = W(t+s) - W(s)$ 是布朗运动**
    ...
    2. **独立增量**：对于任意 $0 \le t_1 < t_2 < \cdots < t_k$，其增量序列为：$B(t_j) - B(t_{j-1}) = W(t_j+s) - W(t_{j-1}+s)$
    ...
    **(2) 证明 $U(t) = cW(t/c^2)$ 是布朗运动**
    ...
    3. **平稳正态增量**：由于线性变换的性质，均值仍为 $0$，方差为：$Var(U(t) - U(s)) = Var\left( c(W(t/c^2) - W(s/c^2)) \right) = c^2 \cdot \left( \frac{t-s}{c^2} \right) = t-s$
...
??? success "解答 (点击展开)"
    ...
    由于 $\min(u,v)$ 在对角线 $u=v$ 处不可微，我们将积分区域 $[0,t] \times [0,t]$ 沿着对角线划分为两块：$u \leqslant v$ 区域和 $u > v$ 区域：
