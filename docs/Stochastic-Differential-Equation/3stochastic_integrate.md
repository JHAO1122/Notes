...
!!! abstract "定义：PWZ 随机积分"
    ...
    利用分部积分公式（Integration by Parts），将 **PWZ 随机积分** 定义为：
...
那我们如何计算 $\int_0^T W(t) dW(t)$？
回到黎曼和的定义，我们能观察到一个在经典微积分中不会出现的现象：**取值点的改变，将导致积分结果的巨大改变。**
...
**两种最重要的积分流派：**
* 当 $\lambda = 0$ 时（取左端点），即为 **Itô 积分**，结果为 $\frac{1}{2}W(T)^2 - \frac{1}{2}T$。它保持了鞅的性质，常用于金融数学领域。
* 当 $\lambda = 1/2$ 时（取中点），即为 **Stratonovich 积分**，校正项相互抵消，结果为 $\frac{1}{2}W(T)^2$，形式上与经典微积分一致，常用于物理和工程领域。
